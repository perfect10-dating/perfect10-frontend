	...
	if (isLoading) {
		...
		if (!user || userReqFailed) {
			...
			if (user.mustReviewDate) {
				...
				if (
					user.lockingDate &&
					new Date(user.lockingDate.time).getTime() > Date.now()
				) {
			...
			if (room.sideTwo.length === 0) {
				isOneSided = true;
				potentialPartners = room.sideOne.filter(
					(sideOneUser) => sideOneUser._id !== user?._id
				);
						...
						<div style={{ maxWidth: "calc(100vw - 140px)" }}>
							<div
								style={{
									...
									textAlign: "center",
									lineHeight: 1,
									marginTop: !isOneSided ? 0 : 5,
								}}
							>
								{!isOneSided && isDisplayingCompetitors
									? "Your Competitors"
									: "Potential Matches"}
							</div>

							{
								...
								!isOneSided && (
									<div
										...
										onClick={() =>
											setIsDisplayingCompetitors(!isDisplayingCompetitors)
										}
									>
										{isDisplayingCompetitors
