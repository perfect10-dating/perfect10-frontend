...
class PhotoCropper extends Component {
	...
	handleSave = async () => {
		...
		let dataUrl = this._cropper.cropper
			.getCroppedCanvas()
			.toDataURL("image/png");
		...
		this.setState({
			...
			imageSize: (file.size / 1000).toFixed(1),
		});
		...
		this.setState({
			saving: true,
			showUploadLoading: true,
			imageSize: (compressedFile.size / 1000).toFixed(1),
		});
				...
				<div className="photo-cropper-overlay">
					<div>
						<div className="loading-box">
							...
							<p>
								For optimal performance, please use photo smaller than 512kb
							</p>
						</div>
						<CropSubmit
							style={{
	...
	render() {
		...
		return (
			<div style={{ color: "lightgray", textAlign: "center", marginTop: 65 }}>
				{this.state.showUploadLoading ? (
					...
					<div>
						...
						>
							<CropSubmit onClick={this.handleSave}>Save</CropSubmit>
